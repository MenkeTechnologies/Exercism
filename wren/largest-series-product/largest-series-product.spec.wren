...
Testie.test("LargestSeriesProduct") { |do|
  ...
  do.test("can find the largest product of 2 with numbers in order") {
    var actual = Series.new("0123456789", 2).largestProduct
    var expected = 72
    Expect.value(actual).toEqual(expected)
  }

  do.test("can find the largest product of 2") {
    var actual = Series.new("576802143", 2).largestProduct
    var expected = 48
    Expect.value(actual).toEqual(expected)
  }

  do.test("can find the largest product of 3 with numbers in order") {
    var actual = Series.new("0123456789", 3).largestProduct
    var expected = 504
    Expect.value(actual).toEqual(expected)
  }

  do.test("can find the largest product of 3") {
    var actual = Series.new("1027839564", 3).largestProduct
    var expected = 270
    Expect.value(actual).toEqual(expected)
  }

  do.test("can find the largest product of 5 with numbers in order") {
    var actual = Series.new("0123456789", 5).largestProduct
    var expected = 15120
    Expect.value(actual).toEqual(expected)
  }

  do.test("can get the largest product of a big number") {
    var actual = Series.new("73167176531330624919225119674426574742355349194934", 6).largestProduct
    var expected = 23520
    Expect.value(actual).toEqual(expected)
  }

  do.test("reports zero if the only digits are zero") {
    var actual = Series.new("0000", 2).largestProduct
    var expected = 0
    Expect.value(actual).toEqual(expected)
  }

  do.test("reports zero if all spans include zero") {
    var actual = Series.new("99099", 3).largestProduct
    var expected = 0
    Expect.value(actual).toEqual(expected)
  }

  do.test("rejects span longer than string length") {
    Expect.that {
      Series.new("123", 4).largestProduct
    }.abortsWith("span must be smaller than string length")
  }

  do.test("reports 1 for empty string and empty product (0 span)") {
    var actual = Series.new("", 0).largestProduct
    var expected = 1
    Expect.value(actual).toEqual(expected)
  }

  do.test("reports 1 for nonempty string and empty product (0 span)") {
    var actual = Series.new("123", 0).largestProduct
    var expected = 1
    Expect.value(actual).toEqual(expected)
  }

  do.test("rejects empty string and nonzero span") {
    Expect.that {
      Series.new("", 1).largestProduct
    }.abortsWith("span must be smaller than string length")
  }

  do.test("rejects invalid character in digits") {
    Expect.that {
      Series.new("1234a5", 2).largestProduct
    }.abortsWith("digits input must only contain digits")
  }

  do.test("rejects negative span") {
    Expect.that {
