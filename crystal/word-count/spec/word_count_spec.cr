...
describe "WordCount" do
  ...
  pending "count one of each word" do
    sentence = "one of each"
    expected = {"one" => 1, "of" => 1, "each" => 1}
    WordCount.count_words(sentence).should eq(expected)
  end

  pending "multiple occurrences of a word" do
    sentence = "one fish two fish red fish blue fish"
    expected = {"one" => 1, "fish" => 4, "two" => 1, "red" => 1, "blue" => 1}
    WordCount.count_words(sentence).should eq(expected)
  end

  pending "handles cramped lists" do
    sentence = "one,two,three"
    expected = {"one" => 1, "two" => 1, "three" => 1}
    WordCount.count_words(sentence).should eq(expected)
  end

  pending "handles expanded lists" do
    sentence = "one,\ntwo,\nthree"
    expected = {"one" => 1, "two" => 1, "three" => 1}
    WordCount.count_words(sentence).should eq(expected)
  end
  ...
  pending "multiple spaces not detected as a word" do
    sentence = " multiple   whitespaces"
    expected = {"multiple" => 1, "whitespaces" => 1}
    WordCount.count_words(sentence).should eq(expected)
  end

  pending "alternating word separators not detected as a word" do
    sentence = ",\n,one,\n ,two \n 'three'"
    expected = {"one" => 1, "two" => 1, "three" => 1}
    WordCount.count_words(sentence).should eq(expected)
  end

  pending "quotation for word with apostrophe" do
