...
describe "VariableLengthQuantity" do
  describe "Encode a series of integers, producing a series of bytes." do
    ...
    it "arbitrary single byte" do
      input = [64]
      expected = [64]
      VariableLengthQuantity.encode(input).should eq(expected)
    end

    it "largest single byte" do
      input = [127]
      expected = [127]
      VariableLengthQuantity.encode(input).should eq(expected)
    end

    it "smallest double byte" do
      input = [128]
      expected = [129, 0]
      VariableLengthQuantity.encode(input).should eq(expected)
    end

    it "arbitrary double byte" do
      input = [8192]
      expected = [192, 0]
      VariableLengthQuantity.encode(input).should eq(expected)
    end

    it "largest double byte" do
      input = [16383]
      expected = [255, 127]
      VariableLengthQuantity.encode(input).should eq(expected)
    end

    it "smallest triple byte" do
      input = [16384]
      expected = [129, 128, 0]
      VariableLengthQuantity.encode(input).should eq(expected)
    end

    it "arbitrary triple byte" do
      input = [1048576]
      expected = [192, 128, 0]
      VariableLengthQuantity.encode(input).should eq(expected)
    end

    it "largest triple byte" do
      input = [2097151]
      expected = [255, 255, 127]
      VariableLengthQuantity.encode(input).should eq(expected)
    end

    it "smallest quadruple byte" do
      input = [2097152]
      expected = [129, 128, 128, 0]
      VariableLengthQuantity.encode(input).should eq(expected)
    end

    it "arbitrary quadruple byte" do
      input = [134217728]
      expected = [192, 128, 128, 0]
      VariableLengthQuantity.encode(input).should eq(expected)
    end

    it "largest quadruple byte" do
      input = [268435455]
      expected = [255, 255, 255, 127]
      VariableLengthQuantity.encode(input).should eq(expected)
    end

    it "smallest quintuple byte" do
      input = [268435456]
      expected = [129, 128, 128, 128, 0]
      VariableLengthQuantity.encode(input).should eq(expected)
    end

    it "arbitrary quintuple byte" do
      input = [4278190080]
      expected = [143, 248, 128, 128, 0]
      VariableLengthQuantity.encode(input).should eq(expected)
    end

    it "maximum 32-bit integer input" do
      input = [4294967295]
      expected = [143, 255, 255, 255, 127]
      VariableLengthQuantity.encode(input).should eq(expected)
    end

    it "two single-byte values" do
      input = [64, 127]
      expected = [64, 127]
      VariableLengthQuantity.encode(input).should eq(expected)
    end

    it "two multi-byte values" do
      input = [16384, 1193046]
      expected = [129, 128, 0, 200, 232, 86]
      VariableLengthQuantity.encode(input).should eq(expected)
    end

    it "many multi-byte values" do
  ...
  describe "Decode a series of bytes, producing a series of integers." do
    ...
    it "maximum 32-bit integer" do
      input = [143, 255, 255, 255, 127]
      expected = [4294967295]
      VariableLengthQuantity.decode(input).should eq(expected)
    end

    it "incomplete sequence causes error" do
      expect_raises(ArgumentError) do
        VariableLengthQuantity.decode([255])
      end
    end

    it "incomplete sequence causes error, even if value is zero" do
      expect_raises(ArgumentError) do
